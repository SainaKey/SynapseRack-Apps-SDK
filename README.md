...
ノードベースVJ環境 **[SynapseRack](https://synapserack.com/ja/)** の中で動く、ウィンドウ付きJavaScriptアプリを作るためのSDKです。アプリの実体はただのHTML/JSフォルダ — ビルド不要、ツールチェーン不要、保存すれば即ホットリロード。アプリは `window.synapse` ブリッジを通じてホストと会話し、メディア再生、オフスクリーン合成、ユーザーのノードグラフへのテクスチャ公開、MIDI / LFO / オーディオへのパラメータバインドができます。
